...
Console.CancelKeyPress += (_, eventArgs) =>
{
    ...
    AnsiConsole.MarkupLine("[yellow]Ctrl+C는 무시됩니다. 종료하려면 [cyan]/exit[/]를 입력하세요.[/]");
};
...
var consolePermissionEnforcer = invocation.IsNonInteractive ? null : new ConsolePermissionEnforcer();
...
var commandDispatcher = new InteractiveCommandDispatcher(new InteractiveCommandCallbacks
{
    ...
    Help = () => ShowHelp(toolRegistry, providerFactory.AvailableProviders),
    Setup = async () => provider = await RunSetupAsync(config, providerFactory, toolRegistry),
...
static async Task<ILlmProvider> RunSetupAsync(ProviderConfiguration config, ProviderFactory providerFactory, ToolRegistry registry)
{
    AnsiConsole.Write(new Rule { Title = "Setup", Style = Style.Parse("green") });

    var providers = providerFactory.AvailableProviders.OrderBy(name => name).ToArray();
...
static async Task SendAndDisplay(ILlmProvider provider, string model, uint maxTokens, ChatConversationHistory history, ToolRegistry registry, IPermissionEnforcer enforcer, CliToolLoopRunner loopRunner, CancellationToken ct = default)
{
...
static CancellationTokenSource? CreateTimeoutCancellation(int timeoutSeconds)
{
...
static string FormatTimeout(int timeoutSeconds)
{
    return timeoutSeconds <= 0
        ? "[cyan]disabled[/]"
...
static void ShowOrUpdatePermissions(ConsolePermissionEnforcer? enforcer, string argument)
{
    if (enforcer == null)
    {

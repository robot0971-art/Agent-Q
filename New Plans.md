...
## 1. Dynamic Prompt Assembly v1
- Classify request type: bug fix, feature, review, verification failure, documentation, analysis, refactor.
- Assemble task-specific system hints, context shape, and tool priority.
- Keep prompts compact and avoid sending irrelevant workspace context.

## 2. Memory v2 / Context Bank
- Structure project memory into stack, rules, preferences, forbidden patterns, key commands, key files, key symbols, and recurring errors.
- Query only relevant memory for each task.
- Keep the current JSON approach first, then prepare for SQLite or graph-style storage later.

## 3. TypeScript / JavaScript Worker v1

...
Repair instructions are case-specific. Agent Q classifies missing npm scripts, missing dependencies, dev-server startup failures, React runtime errors, import/export mismatches, JSX syntax errors, blank or broken screens, missing DOM requirements, and mobile visual layout failures. Each strategy names priority files, preferred repair actions, actions to avoid, and verification commands to rerun. Package/script/dependency repairs still go through normal workspace validation, permission checks, and replay evidence.

For confident Vite/React package failures, `FrontendPackageRepairService` performs a narrow deterministic repair before the model repair prompt: it can add missing `scripts.dev`, `scripts.build`, `dependencies.react`, `dependencies.react-dom`, `devDependencies.vite`, and `devDependencies.@vitejs/plugin-react` without overwriting existing values. The package edit is recorded through file mutation snapshots and `frontend_package_repair` replay evidence. The repair is not completion evidence by itself; Agent Q must still run the permissioned install/build/preview path and collect successful runtime preview evidence before reporting success.

...
- [x] 명확한 명령형 실행 표면, 예를 들어 “만들어줘”, “생성해줘”, “실행해줘”, “테스트 돌려줘”만 deterministic fallback이 실행 요청으로 보존하게 한다.
- [x] 상담형/방법형/가능성 질문, 예를 들어 “만들 수 있을까”, “뭐가 좋을까”, “how do I”, “what would be good”은 LLM `Conversation` 판단을 우선하게 한다.
- [x] `Ambiguous` turn은 no-tool 실패 메시지가 아니라 한 가지 확인 질문 또는 구체 옵션 제시로 끝나게 한다.
- [x] 새 프로젝트 생성, 폴더 생성, 파일 수정, 빌드, 테스트, 로컬 서버 실행은 기존 deterministic Desktop service 경로를 유지한다.
- [x] 실행형 요청에서 LLM이 말로만 “완료했습니다”라고 답하는 경로는 계속 실패/재시도 처리하고 tool/service replay evidence를 요구한다.
- [x] 과거 대화, session summary, checkpoint, memory, scaffold hint, verification hint, pasted log/example이 최신 사용자 요청을 덮지 않는지 router 단계에서 다시 검증한다.
- [x] RAG 계열 context, 즉 `WorkspaceIndexer`, `EmbeddingIndexBuilder`/`EmbeddingIndexStore`, `semantic_search`, `hybrid_search`, `ProjectMemory`, `ExecutionLessonMemory`가 최신 사용자 요청보다 위에 서거나 실행 intent를 부당하게 밀어붙이지 않는지 다시 검증한다.
- [x] embedding index stale chunk, 오래된 memory lesson, hybrid search ranking 결과가 `Conversation` turn을 `Action`처럼 오염시키지 않는 실제 RAG 구성요소 회귀 테스트를 추가한다.
...
- [x] 3차 Preview / DOM / Screenshot / Visual QA 1차 게이트
  ...
  - [x] `ImplementationRuntimePreviewService`를 추가해 source check 통과 후 preview evidence가 없으면 `DesktopLocalServerService`로 localhost preview를 시작하고 DOM snapshot을 `.agentq/preview/*-dom.html`에 저장한다.
  - [x] runtime preview 결과는 `implementation_runtime_preview` replay evidence로 남기고 final guard가 URL/DOM 결과를 completion evidence로 사용한다.
  - [x] Playwright/browser 기반 desktop/mobile screenshot artifact 생성과 console error 직접 수집은 후속 강화가 필요하다.
  - [x] `PlaywrightImplementationPreviewBrowserVerifier`를 추가해 workspace-local Playwright로 localhost preview를 열고 desktop/mobile screenshot을 `.agentq/preview/`에 저장한다.
  - [x] browser-rendered DOM, console error, page error, screenshot artifact, visual finding을 `ImplementationPreviewVerificationResult`와 `implementation_runtime_preview` replay evidence에 연결했다.
  - [x] Playwright가 없거나 screenshot/console/visual evidence가 실패하면 preview completion을 실패 처리해 blank/dark/깨진 화면을 완료라고 말하지 못하게 했다.
- [x] 4차 Preview failure repair retry
...
- [x] 6차 케이스별 자동 복구 전략
  - [x] missing npm script, missing dependency, dev server failure, React runtime error, import/export mismatch, JSX syntax error, blank/broken screen, missing DOM, mobile visual layout failure를 별도 repair kind로 분류한다.
  - [x] 분류별 repair instruction에 priority files, repair actions, avoid actions, re-run commands를 포함한다.
  - [x] dependency/package/script 복구는 `package.json` 우선 확인, `npm install`/`npm run build`/`npm run dev` 재검증, workspace/permission/replay evidence 보존을 명시한다.
  - [x] React runtime/import/JSX 복구는 관련 `src/*` 파일 우선 확인, placeholder 대체 금지, build/preview 재검증을 명시한다.
  - [x] blank/dark/mobile visual 복구는 mount/CSS/responsive layout/screenshot 재검증 전략을 명시한다.
- [x] 7차 package/script/dependency deterministic repair
  - [x] `FrontendPackageRepairService`를 추가해 확실한 Vite/React workspace에서만 root `package.json`을 구조적으로 파싱하고 패치한다.
  - [x] missing npm script/dependency preview failure에서 `scripts.dev`, `scripts.build`, `dependencies.react`, `dependencies.react-dom`, `devDependencies.vite`, `devDependencies.@vitejs/plugin-react` 누락을 진단한다.
  - [x] 기존 script/dependency 값은 덮어쓰지 않고 누락 필드만 추가한다.
  - [x] framework evidence가 부족하거나 `package.json`이 없거나 malformed이면 silent write 없이 기존 A7 repair instruction 경로로 fallback한다.
  - [x] deterministic package patch를 `DesktopAgentService` runtime preview repair loop에 연결하고 `frontend_package_repair` replay evidence와 file mutation snapshot을 남긴다.
  - [x] package patch 자체를 완료 evidence로 보지 않고, permission 기반 `npm install`/`npm run build`/`npm run dev` 및 runtime preview 재검증을 다음 loop에 요구한다.
- [x] 회귀 테스트
  ...
  - [x] runtime preview service가 Node dev server를 시작하고 localhost DOM evidence/snapshot/replay를 남기는 테스트를 추가했다.
  - [x] runtime preview service가 browser screenshot evidence를 replay에 남기고 console/visual failure를 완료 차단 evidence로 쓰는 테스트를 추가했다.
  ...
  - [x] runtime preview와 build/test failure가 케이스별 repair strategy를 포함하는지 테스트를 추가했다.
  - [x] missing script/dependency, React runtime error, import/export mismatch, JSX syntax error, mobile visual layout failure 분류 테스트를 추가했다.
  - [x] Vite/React package repair가 누락 scripts/dependencies를 추가하고 기존 값을 덮어쓰지 않으며 framework가 불명확하면 skip하는 테스트를 추가했다.
  - [x] DesktopAgentService deterministic package repair가 replay evidence와 file mutation snapshot을 기록하는 테스트를 추가했다.
...
- [x] 2026-06-18 A4 focused 검증
  - [x] `dotnet build csharp\AgentQ.Tests\AgentQ.Tests.csproj --no-restore /p:UseSharedCompilation=false /p:NodeReuse=false /m:1 --verbosity:minimal` 통과, 경고 0, 오류 0.
  - [x] `dotnet test csharp\AgentQ.Tests\AgentQ.Tests.csproj --no-build --filter "FullyQualifiedName~ImplementationRuntimePreviewService|FullyQualifiedName~PlaywrightImplementationPreviewBrowserVerifier" --logger "console;verbosity=normal" /p:UseSharedCompilation=false /p:NodeReuse=false /m:1` exit code 0.
  - [x] `dotnet build csharp\AgentQ.Desktop\AgentQ.Desktop.csproj --no-restore /p:UseSharedCompilation=false /p:NodeReuse=false /m:1 --verbosity:minimal` 통과, 경고 0, 오류 0.
  - [x] `dotnet test csharp\AgentQ.Tests\AgentQ.Tests.csproj --no-build --logger "trx;LogFileName=agentq-a4-full.trx" --results-directory csharp\AgentQ.Tests\TestResults --verbosity:minimal /p:UseSharedCompilation=false /p:NodeReuse=false /m:1` 통과 1136, 실패 0, 건너뜀 0.
- [x] 2026-06-18 A5 focused 검증
  - [x] `dotnet build csharp\AgentQ.Tests\AgentQ.Tests.csproj --no-restore /p:UseSharedCompilation=false /p:NodeReuse=false /m:1 --verbosity:minimal` 통과, 경고 0, 오류 0.
...
- [x] 2026-06-18 A7 focused 검증
  - [x] `dotnet build csharp\AgentQ.Tests\AgentQ.Tests.csproj --no-restore /p:UseSharedCompilation=false /p:NodeReuse=false /m:1 --verbosity:minimal` 통과, 경고 0, 오류 0.
  - [x] `dotnet test csharp\AgentQ.Tests\AgentQ.Tests.csproj --no-build --filter "FullyQualifiedName~ClassifiesRuntimePreviewRepairCases|FullyQualifiedName~ClassifiesMobileVisualLayoutFailure|FullyQualifiedName~ClassifiesBuildTestRepairCases|FullyQualifiedName~BuildsRuntimePreviewRepairInstruction|FullyQualifiedName~BuildsFailedBuildTestRepairInstruction|FullyQualifiedName~DesktopAgentService_SmokeLuxuryShopScaffoldContinuesIntoImplementationAndPreviewGate|FullyQualifiedName~RuntimePreviewRepair|FullyQualifiedName~FailedBuildTestRepair|FullyQualifiedName~SuccessfulRuntimePreviewReplayEvidence|FullyQualifiedName~ClassifiesRuntimePreviewFailuresAndStopsRepeatedRepair|FullyQualifiedName~RetriesImplementationAfterRuntimePreviewFailure" --logger "console;verbosity=normal" /p:UseSharedCompilation=false /p:NodeReuse=false /m:1` 통과 16, 실패 0.
  - [x] `dotnet build csharp\AgentQ.Desktop\AgentQ.Desktop.csproj --no-restore /p:UseSharedCompilation=false /p:NodeReuse=false /m:1 --verbosity:minimal` 통과, 경고 0, 오류 0.
  - [x] `dotnet test csharp\AgentQ.Tests\AgentQ.Tests.csproj --no-build --logger "trx;LogFileName=agentq-a7-full.trx" --results-directory csharp\AgentQ.Tests\TestResults --verbosity:minimal /p:UseSharedCompilation=false /p:NodeReuse=false /m:1`는 wrapper timeout 124를 반환했지만, 생성된 `agentq-a7-full.trx`는 `Completed`, 통과 1151, 실패 0, 건너뜀 0으로 기록됐다.
- [x] 2026-06-18 A8 focused 검증
  - [x] `dotnet build csharp\AgentQ.Tests\AgentQ.Tests.csproj --no-restore /p:UseSharedCompilation=false /p:NodeReuse=false /m:1 --verbosity:minimal` 통과, 경고 0, 오류 0.
  - [x] `dotnet test csharp\AgentQ.Tests\AgentQ.Tests.csproj --no-build --filter "FullyQualifiedName~FrontendPackageRepairService|FullyQualifiedName~DesktopAgentService_DeterministicPackageRepairRecordsReplayAndFileChange" --logger "console;verbosity=normal" /p:UseSharedCompilation=false /p:NodeReuse=false /m:1` 통과 5, 실패 0.
  - [x] `dotnet build csharp\AgentQ.Desktop\AgentQ.Desktop.csproj --no-restore /p:UseSharedCompilation=false /p:NodeReuse=false /m:1 --verbosity:minimal` 통과, 경고 0, 오류 0.
  - [x] `dotnet test csharp\AgentQ.Tests\AgentQ.Tests.csproj --no-build --filter "FullyQualifiedName~ClassifiesRuntimePreviewRepairCases|FullyQualifiedName~ClassifiesBuildTestRepairCases|FullyQualifiedName~BuildsRuntimePreviewRepairInstruction|FullyQualifiedName~BuildsFailedBuildTestRepairInstruction|FullyQualifiedName~FrontendPackageRepairService|FullyQualifiedName~DesktopAgentService_DeterministicPackageRepairRecordsReplayAndFileChange" --logger "console;verbosity=normal" /p:UseSharedCompilation=false /p:NodeReuse=false /m:1` 통과 16, 실패 0.
...
dotnet test csharp\AgentQ.Tests\AgentQ.Tests.csproj --filter "FullyQualifiedName~DesktopAgentService_AllowsIdentityConversationWithoutNoToolGuard|FullyQualifiedName~DesktopAgentService_AllowsHowToConversationWithoutShellExecution|FullyQualifiedName~DesktopAgentService_PrefersLlmConversationForConsultativeFeatureProfile|FullyQualifiedName~DesktopAgentService_ReturnsClarificationForAmbiguousProjectRequest|FullyQualifiedName~DesktopAgentService_DoesNotBuildTaskContractWhenModelPromotesConversationToAction|FullyQualifiedName~DesktopAgentService_DirectFallbackCreatesFolderWhenModelAnswersIrrelevantText|FullyQualifiedName~DesktopAgentService_E2eCreatesDirectoryFromExplicitCommand|FullyQualifiedName~DesktopAgentService_LlmFirstRegressionRetriesFalseSuccessThenExecutesContractTool|FullyQualifiedName~DesktopAgentService_BuildContextUsesRouterContractInsteadOfRetranslatingRagLikeText|FullyQualifiedName~UserTurnUnderstanding_PrefersModelConversationForConsultativeFallbackAction|FullyQualifiedName~UserTurnUnderstanding_PreservesConcreteCreateDirectoryWhenModelSaysConversation|FullyQualifiedName~UserTurnUnderstanding_PreservesCurrentActionBeforePastedContext|FullyQualifiedName~UserIntentTranslator_RecognizesConcreteReactSiteProjectRequest|FullyQualifiedName~UserIntentTranslator_RecognizesCurrentFolderCreateDirectoryRequest|FullyQualifiedName~TurnIntentClassifier_ClassifiesConversationActionHybridAndAmbiguous|FullyQualifiedName~DesktopAgentService_DoesNotExecuteEmbeddedFolderCommandInBadResponseComplaint|FullyQualifiedName~DesktopAgentService_CreatesExplicitFolderWhenPastedIrrelevantAnswerFollowsRequest|FullyQualifiedName~DesktopAgentService_BlocksToolCallForEmbeddedCommandEvenWhenLlmIntentSaysAction|FullyQualifiedName~LlmFirstIntentRouter_|FullyQualifiedName~DesktopSemanticSearchTool_|FullyQualifiedName~DesktopHybridSearchTool_" -v minimal
# 2026-06-14 결과: 통과 59, 실패 0, 건너뜀 0, 전체 59.

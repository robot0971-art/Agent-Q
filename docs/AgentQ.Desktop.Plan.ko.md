...
- Settings, Chat, Project, Git, 검증, Plan, Memory, File Change Review, Run Timeline 패널의 UserControl 분리
...
현재 Desktop 기능은 `MainWindow.xaml`과 `MainWindow.xaml.cs`에서 점진적으로 분리 중입니다. Settings, Chat, Project, Git, 검증, Plan, Memory, File Change Review, Run Timeline 패널은 이미 UserControl로 분리되었습니다. 다음 단계에서는 브랜치 복구 UX를 보강하고, `MainWindow.xaml.cs`의 조정 로직을 focused workflow/service로 더 옮겨 변경 범위를 줄입니다.

...
The desktop app includes an initial `semantic_search` tool for OpenAI-compatible embedding indexes. It searches `.agentq/embeddings/chunks.jsonl` by cosine similarity after an embedding vector index has been built. Use the Project panel's `Build embedding index` button to create the local vector index for OpenAI or OpenCode Go.
